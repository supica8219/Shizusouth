...
for(i=0;i<AINO.length;i++){
 if(AINO[i][0]==Hour){
   if(AINO[i][1]>Min){
     ...
     p.innerHTML=AINO[i][0]+":"+String(AINO[i][1]).padStart(2, '0');
     p.classList.add("Nextp");
     AINOTime[0].appendChild(p);
     VC++;
   }
 }
 if(AINO[i][0]>Hour){
...
for(i=0;i<DAIGAKU.length;i++){
 if(DAIGAKU[i][0]==Hour){
   if(DAIGAKU[i][1]>Min){
     var p=document.createElement("p");
     p.innerHTML=DAIGAKU[i][0]+":"+String(DAIGAKU[i][1]).padStart(2, '0');
     p.classList.add("Nextp");
     DAIGAKUTime[0].appendChild(p);
     VC++;
   }
 }
 if(DAIGAKU[i][0]>Hour){
